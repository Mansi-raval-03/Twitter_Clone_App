  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream:
            FirebaseFirestore.instance.collection('tweets').doc(widget.tweet.id).snapshots(),
        builder: (context, snap) {
          if (snap.hasError) {
            return Center(child: Text('Error loading tweet: ${snap.error}'));
          }

          final tweetDoc = snap.data;
          final TweetModel tweet =
              (tweetDoc != null && tweetDoc.exists) ? TweetModel.fromDoc(tweetDoc) : widget.tweet;

          // Prefer original fields for retweets when present
          final isRetweet = tweet.isRetweet;
          final displayUsername = isRetweet ? (tweet.originalUsername.isNotEmpty ? tweet.originalUsername : tweet.username) : tweet.username;
          final displayHandle = isRetweet ? (tweet.originalHandle.isNotEmpty ? tweet.originalHandle : tweet.handle) : tweet.handle;
          final displayProfileImage = isRetweet ? (tweet.originalProfileImage.isNotEmpty ? tweet.originalProfileImage : tweet.profileImage) : tweet.profileImage;
          final displayContent = isRetweet ? (tweet.originalContent.isNotEmpty ? tweet.originalContent : tweet.content) : tweet.content;
          final displayImage = isRetweet ? (tweet.originalImageUrl.isNotEmpty ? tweet.originalImageUrl : tweet.imageUrl) : tweet.imageUrl;
              ...
              children: [
                ...
                Padding(
                  ...
                  child: FutureBuilder<DocumentSnapshot<Map<String, dynamic>>?>(
                    future: (tweet.uid.isNotEmpty)
                        ? FirebaseFirestore.instance.collection('users').doc(tweet.uid).get()
                        : Future.value(null),
                    builder: (ctx, userSnap) {
                      final udata = userSnap.data?.data();
                      final authorName = (udata != null && (udata['name'] as String?)?.trim().isNotEmpty == true)
                          ? udata['name'].toString()
                          : displayUsername;
                      final rawHandle = (udata != null) ? (udata['username'] as String?)?.toString() ?? displayHandle : displayHandle;
                      final authorHandle = rawHandle.startsWith('@') ? rawHandle : '@$rawHandle';
                      final authorProfileImage = (udata != null) ? (udata['profileImage'] as String?)?.toString() ?? displayProfileImage : displayProfileImage;
                      ...
                      return Row(
                        ...
                        children: [
                          GestureDetector(
                            onTap: () {
                              if (tweet.uid.isNotEmpty) {
                                Get.to(() => UserProfileScreen(viewedUserId: tweet.uid));
                              }
                            },
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(_formatDate(displayCreatedAt),
                          ...
                          Builder(builder: (ctx2) {
                            ...
                            if (isFollowing) {
                              return OutlinedButton(
                                ...
                                child: Text('Following', style: TextStyle(color: Theme.of(context).textTheme.bodyLarge?.color)),
                              );
                            }
                ...
                Padding(
                    ...
                    child: Text(displayContent, style: const TextStyle(fontSize: 18, height: 1.5))),
                if (displayImage.trim().isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.only(top: 12, left: 16, right: 16),
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(16),
                      child: AspectRatio(aspectRatio: 16 / 9, child: _buildTweetImage(displayImage)),
                    ),
                  ),
                ...
                Padding(
                  ...
                  child: Text('Replies', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Theme.of(context).textTheme.bodyLarge?.color)),
                ),
                ...
                StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (snapshot.hasError) {
                      return Padding(padding: const EdgeInsets.all(16), child: Text('Error loading replies: ${snapshot.error}'));
                    }
                    final docs = snapshot.data?.docs ?? [];
                    if (docs.isEmpty) {
                      return Padding(
                        padding: const EdgeInsets.all(16),
                        child: Center(child: Text('No replies yet. Be the first to reply!', style: TextStyle(color: Colors.grey[600]))),
                      );
                    }
                    final replies = docs.map((doc) => ReplyModel.fromDoc(doc)).toList();
                ...
                children: [
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: InputDecoration(hintText: 'Tweet your reply', border: InputBorder.none, hintStyle: TextStyle(color: Colors.grey[600])),
                      maxLines: 4,
                      autofocus: true,
                    ),
                  ),
